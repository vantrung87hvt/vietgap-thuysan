...
*  edited by invi.vn@gmail.com on 09.06.2008
*/
...
[
[ "#dlcalendar_container",         'width:12em; table-layout:fixed;' ],
[ '#dlcalendar_navigationRow',     'height:1.5em; width:100%; margin:0px; border:1px solid #000000; background-color:#0066FF; color:#ffffff; font-family:arial,helvetica,sans-serif; text-align:center;  cursor:default;' ],
...
[ '#dlcalendar_headerRow',         'height:1.2em; text-align:center; vertical-align:middle; color:#ffffff; background-color:#E2F4FE; font-family:arial,helvetica,sans-serif; cursor:default;' ],
...
function dlcalendar_writeHeaderDate()
{
	...
	var currentYear=dDisplayedDate.getFullYear();
	var str="<select id='cboYear'>";
	for(i=currentYear-10;i<currentYear+10;i++)
	{
		str+="<option style='width:30px;' value='"+i+"'";
		if(i==currentYear)str+=" selected ";
		str+=">";
		str+=i;
		str+="</option>";
	}
	str+="</select>";
	dlcalendar_getElementById( this.sDisplayedMonthCellId ).innerHTML =( dDisplayedDate.dlcalendar_mGetMonthString( this.aMonths ) + str);
...
function dlcalendar_dateObjectToString( dDate )
{
	...
	for( var i=0; i<nMatches; i++ )
	{
		...
		{
			case 'y':
				sYear =document.getElementById('cboYear').value;// dDate.getFullYear().toString(); $edited
				if( nChars <= 2 )
				{
