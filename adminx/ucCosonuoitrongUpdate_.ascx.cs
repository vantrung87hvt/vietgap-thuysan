...
public partial class adminx_ucCosonuoitrongUpdate : System.Web.UI.UserControl
{
    ...
    {        
        string body = INVIHelper.ReadFile("upload/emailactive.html");           
        body = body.Replace("##UserName##", strUsername);
        body = body.Replace("##URL##", "ctms.thanglongsport.vn/active.aspx?code=" + INVISecurity.MD5(txtUsername.Text));
        INVIHelper.Send_Email(txtEmail.Text, "Kích hoạt tài khoản đăng ký tại Viet G.A.P", body);
