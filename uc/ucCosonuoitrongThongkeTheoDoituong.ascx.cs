...
public partial class adminx_BaocaoThongke_ucCosonuoitrongThongkeTheoDoituong : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            napDoituong();
            if ((Session["GroupID"] != null) && (int.Parse(Session["GroupID"].ToString()) == 1))
            {
                lnkXuatraExel.Visible = true;
            }
        }
    }
    ...
    {
        String sRes = "";
        if (!sAp.ToString().Equals(""))
        {
            sRes += sAp.ToString();
            if (!sXa.ToString().Equals(""))
                sRes += ", " + sXa.ToString();
        }
        else
        {
    ...
    protected void btnXem_Click(object sender, EventArgs e)
    {
        ...
        if (PK_iDoituongID > 0)
        {
            ...
            lblTranghthai.Text = "Có " + lstCoso.Count + " hộ nuôi " + ddlDoituongnuoi.SelectedItem.Text + " trên tổng số " + iTongso.ToString() + " hộ.";
            lblDoituongnuoi.Text = ddlDoituongnuoi.SelectedItem.Text;
